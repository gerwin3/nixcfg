...
* ~~Flickering on Sway with NVIDIA: Sway's latest release is based on wlroots 0.17 which does not have explicit sync support. This will be fixed in the next Sway release which will be based on wlroots 0.19 (0.18 and above have explicit sync). [Sway 1.10 release candidates are already coming out](https://github.com/swaywm/sway/releases) so it will not be long until 1.10 is there.~~ Currently on a recent Sway master branch revision to work around tearing issue.
